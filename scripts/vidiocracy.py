...
from scripts.logging_vidiocracy import logger_vidiocracy
...
class VidiocracyScript(scripts.Script):
    motion_module: MotionWrapper = None
    
    def __init__(self):
        self.logger = logger_vidiocracy
    ...
    def unload_motion_module(self):
        ...
        if VidiocracyScript.motion_module is not None:
            VidiocracyScript.motion_module.to(cpu)
        torch_gc()
        gc.collect()

    def remove_motion_module(self):
        self.logger.info("Removing motion module from any memory")
        del VidiocracyScript.motion_module
        VidiocracyScript.motion_module = None
        torch_gc()
        gc.collect()
    
    def ui(self, is_img2img):
        with gr.Accordion('Vidiocracy', open=False):
            model = gr.Dropdown(choices=["mm_sd_v15.ckpt", "mm_sd_v14.ckpt"], value="mm_sd_v15.ckpt", label="Motion module", type="value")
            with gr.Row():
                enable = gr.Checkbox(value=False, label='Enable Vidiocracy')
    ...
    def inject_motion_modules(self, p: StableDiffusionProcessing, model_name="mm_sd_v15.ckpt"):
        model_path = os.path.join(shared.opts.data.get("vidiocracy_model_path", os.path.join(script_dir, "model")), model_name)
        if not os.path.isfile(model_path):
            raise RuntimeError("Please download models manually.")
        if VidiocracyScript.motion_module is None or VidiocracyScript.motion_module.mm_type != model_name:
            if shared.opts.data.get("vidiocracy_check_hash", True):
                ...
                if hashes.sha256(model_path, f"Vidiocracy/{model_name}") != get_mm_hash(model_name):
            ...
            VidiocracyScript.motion_module = MotionWrapper(model_name)
            missed_keys = VidiocracyScript.motion_module.load_state_dict(mm_state_dict)
            self.logger.warn(f"Missing keys {missed_keys}")
        VidiocracyScript.motion_module.to(device)
        ...
        for mm_idx, unet_idx in enumerate([1, 2, 4, 5, 7, 8, 10, 11]):
            ...
            unet.input_blocks[unet_idx].append(VidiocracyScript.motion_module.down_blocks[mm_idx0].motion_modules[mm_idx1])
        ...
        for unet_idx in range(12):
            ...
            if unet_idx % 3 == 2 and unet_idx != 11:
                unet.output_blocks[unet_idx].insert(-1, VidiocracyScript.motion_module.up_blocks[mm_idx0].motion_modules[mm_idx1])
            else:
                unet.output_blocks[unet_idx].append(VidiocracyScript.motion_module.up_blocks[mm_idx0].motion_modules[mm_idx1])
    ...
    def before_process(self, p: StableDiffusionProcessing, enable_vidiocracy=False, loop_number=0, video_length=16, fps=8, model="mm_sd_v15.ckpt"):
        if enable_vidiocracy:
            self.logger.info(f"Vidiocracy process start with video Max frames {video_length}, FPS {fps}, duration {video_length/fps},  motion module {model}.")
    ...
    def postprocess(self, p: StableDiffusionProcessing, res: Processed, enable_vidiocracy=False, loop_number=0, video_length=16, fps=8, model="mm_sd_v15.ckpt"):
        if enable_vidiocracy:
            self.remove_motion_modules(p)
            video_paths = []
            self.logger.info("Merging images into GIF.")
            from pathlib import Path
            Path(f"{p.outpath_samples}/Vidiocracy").mkdir(exist_ok=True)
            for i in range(res.index_of_first_image, len(res.images), video_length):
                video_list = res.images[i:i+video_length]
                
                for frame_item in video_list:
                    print("FRAMES?: " + str(frame_item))

                seq = images.get_next_sequence_number(f"{p.outpath_samples}/Vidiocracy", "")
                filename = f"{seq:05}-{res.seed}"
                video_path = f"{p.outpath_samples}/Vidiocracy/{filename}.gif"
                video_paths.append(video_path)
                imageio.mimsave(video_path, video_list, duration=(video_length/fps), loop=loop_number)
            res.images = video_paths
            self.logger.info("Vidiocracy process end.")

def on_ui_settings():
    section = ('vidiocracy', "Vidiocracy")
    shared.opts.add_option("vidiocracy_model_path", shared.OptionInfo(os.path.join(script_dir, "model"), "Path to save Vidiocracy motion modules", gr.Textbox, section=section))
    shared.opts.add_option("vidiocracy_check_hash", shared.OptionInfo(True, "Check hash for motion modules. Disable checking if you want to use your own.", gr.Checkbox, section=section))
